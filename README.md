...
The SkinSim experimenter enables automatic configuring and testing of several skin array models. Different model and controller specifications can be generated by modifying and compiling *experimenter/src/auto_generator.cpp*. In future, the generator will use a configuration file to test several parameters over different ranges:
...
where the experiment name is optional. This create an experiment folder in *data/* containing the model and controller configuration files. To run the experimenter:
...
All the CSV files can easily be importeted into MATLAB using the *rtpload.m* script.
