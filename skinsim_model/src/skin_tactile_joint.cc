...
namespace gazebo
{
class Skin_Tactile_Joint : public ModelPlugin
{
...
public:
  ...
  void Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/)
  {
    ...
    this->update_connection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&Skin_Tactile_Joint::UpdateJoint, this));
  }
...
public:
  void UpdateJoint()
  {
    ...
    std::ifstream tact_rec (path.c_str());
    std::vector< std::vector<int> > tactile_array;
    std::string str;
    if(tact_rec.is_open())
    {
      
      while(std::getline(tact_rec, str))
      {
        std::vector <int> row;
        std::stringstream linestream(str);
        int num;
        while (linestream >> num) row.push_back(num); 
        tactile_array.push_back(row); // copies row into Tactile Array
      }
    }
    ...
    double tactile_data ;
    double skin_sum = 0;
    std::vector<double> tact_sum;
    for (unsigned int i = 0; i<tactile_array.size();i++)
    {
      tact_sum.push_back(0.0);
    } 
    //double tact_sum[9];
    ...
    for (unsigned int i = 0; i < this->joints.size(); ++i)
    {
      ...
      for (int p=0; p<tactile_array.size(); p++)
      {
        for (int q = 0; q<tactile_array[p].size(); q++)
        {
          if(tactile_array[p][q] == i)
          {
            tacData.TactElemID.push_back( i );
            tacData.force.push_back( sens_force );
            tact_sum[p] = tact_sum[p]+sens_force;
          }
        }
      }
      ...
      skin_sum = skin_sum + sens_force;
      tact_displacement = tact_displacement + current_angle;

    }
    double total_sens_force = 0; 
    for (unsigned int ix = 0; ix<tactile_array.size();ix++)
    {
      tacData.TactID.push_back(ix+1);
      tacData.tact_ind_force.push_back(tact_sum[ix]);
      total_sens_force = total_sens_force + tact_sum[ix];
    } 
    tacData.tact_total_force = total_sens_force;
    concData.total_force = skin_sum;
...
GZ_REGISTER_MODEL_PLUGIN(Skin_Tactile_Joint)
